package com.kiwilss.wanandroid.http.interceptor
...
class HttpLogInterceptor @JvmOverloads constructor(var printResponseHeader: Boolean = true) :
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        var responseMessage =
            "$responsePrefixStart ${response.code()} ${request.method()} ${
                if (response.message()
                        .isEmpty()
        ...
        if (!HttpHeaders.hasBody(response)) {
            ...
            var buffer = source.buffer as Buffer
            ...
            responseMessage += if (gzippedLength != null) {
                "\n$responsePrefixEnd END HTTP (-byte, $gzippedLength-gzipped-byte body)"
            } else {
                "\n$responsePrefixEnd END HTTP (-byte body)"
            }
        }
